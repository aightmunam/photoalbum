...
@app.task(bind=True)
def log_update_history_task(self, user_id, description, data_to_log):
    """
    Task to create a new UpdateHistory entry using the provided data
    """
    try:
        user = User.objects.filter(id=user_id).first()
        UpdateHistory.objects.create(user=user, description=description, update_values=data_to_log)
